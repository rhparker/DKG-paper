...
\end{equation}
which has an exact static kink solution $u_n = \tanh(a n + \xi)$, where $a = \frac{1}{2} \cosh^{-1}[(1+h^2)/(1-h^2) ]$ and $\xi$ is arbitrary. Although we do not expect this equation to have kink-antikink equilibrium solutions, since the stable and unstable manifolds of the two equilibria do not intersect transversely, we expect that a kink-antikink will be an approximate equilibrium solution. We may then be able to explain the time evolution of the kink-antikink dynamics in terms of interactions between their exponentially decaying tails. Other potential variants we could examine include the sine lattice (SL) form studied in \cite{Takeno1986}. Finally, we could apply similar techniques to study multi-site breathers in discrete Klein-Gordon lattices.
