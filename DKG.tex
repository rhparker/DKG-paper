...
\end{equation}
Considered as an eigenvalue problem for $\lambda^2$, the LHS of \cref{eq:KGevp1} is a self-adjont operator, since it can be represented as a symmetric, infinite dimensional matrix. This implies that $\lambda^2$ is real, which means that $\lambda$ must be either real or pure imaginary. Taking $\omega = \lambda^2$, we re write the eigenvalue problem as
