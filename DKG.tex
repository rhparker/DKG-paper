...
\end{equation}
Thus $S^\pm$ are hyperbolic saddle equilibria of the lattice dynamical system \cref{eq:dynEq}. We take the existence of a stable, symmetric kink (stationary front) as a hypothesis. From the spatial dynamics perspective, this kink solution is a heteroclinic orbit connecting the saddle at $S^-$ to the saddle at $S^+$.
...
\begin{figure}[H]
	...
	\end{tabular}
	\end{center}
	\caption{Left panel shows kink-antikink solution with $N_1 = 8$ for $d = 0.25$. Right panel shows turning point $d_0$ vs $N$ for kink-antikink together with least squares linear regression line.}
	\label{fig:kak}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tabular}
	\end{center}
	\caption{Left panel shows timestepping of perturbation of primary kink solution, where two central nodes of kink are perturbed by 0.1 in same direction. Center and right panels shows difference between perturbed solution and kink for the two central nodes for perturbation of central nodes in same direction (center) and opposite direction (right) by 0.1. Two central nodes are shown in blue solid and red dotted lines. $N=200$ grid points for kink, $d = 0.5$, timestepping using 4th order Runge-Kutta method with step size 0.04.} 
	\label{fig:kinktimestep}
\end{figure}

As we increase the number of nodes $N$ in the lattice, the continuous spectrum fills out the two intervals \cref{eq:contspec}. Resonance then occurs between the internal modes and the continuous spectrum modes, which occurs in the limit as $N\rightarrow \infty$ and causes the oscillations to decay (\cref{fig:kinktimestepresonance}, top). The slope of the linear regression lines on a log-log plot of amplitude versus $t$ suggests that the decay rate of the oscillations is proportional to $t^{-1/2}$, with a relative error of less than $0.02$ (\cref{fig:kinktimestepresonance}, bottom). In addition, the frequency of the in-phase vibrations due to the Goldstone mode is now given by the lower boundary of the continuous spectrum, and the frequency of the out-of-phase vibrations due to the edge mode is now given by the upper boundary of the continuous spectrum, with a relative error of less than $10^{-3}$.

\begin{figure}[H]
	\begin{center}
	\begin{tabular}{cc}
	\includegraphics[width=5cm]{kinkoscgoldstone200.eps} &
	\includegraphics[width=5cm]{kinkoscedge200.eps} \\
	\includegraphics[width=5cm]{kinkampvstgoldstone.eps} &
	\includegraphics[width=5cm]{kinkampvstedge.eps} 
	\end{tabular}
	\end{center}
	\caption{Difference between perturbed solution and kink for the two central nodes for perturbation of central nodes in same direction (top left) and opposite direction (top right) by 0.1. Two central nodes are shown in blue solid and red dotted lines. On bottom are corresponding plots for log of amplitude of oscillations vs. $\log t$ together with linear regression line. $N=200$ grid points for kink, $d = 0.5$, timestepping using 4th order Runge-Kutta method with step size 0.04.} 
