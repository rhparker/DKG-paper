...
\end{equation*}
% PGK: $d$ is a slightly unfortunate selection for the 
% coupling for which often $C$ is used, but I won't change that now.
describes the dynamics of an infinitely long, one-dimensional lattice of particles which are harmonically coupled to their neighbors through the discrete second difference operator $\Delta_2$ and are subject to an external, nonlinear, on-site potential $f(u) = P'(u)$ \cite{Karachalios}. The quantity $u_n$ represents the displacement of the particle at site $n$ in the lattice, $d$ is the strength of the nearest neighbor coupling, and the dot denotes the derivative with respect to the time $t$. 

A specific example is the discrete sine-Gordon equation
\begin{equation}\label{eq:dSG}
	\ddot{u}_n = d (\Delta_2 u)_n - \sin(u_n),
\end{equation}
in which the external potential is periodic. This equation is also known as the Frenkel-Kontorova model, and was introduced in 1938 to describe the dynamics of a crystal lattice near a dislocation core \cites{braun1998,braun2004}. This equation has since been used in numerous applications, including a mechanical model for a chain of pendula coupled with elastic springs \cite{Scott1969,english}, arrays of Josephson junctions \cites{Ustinov1992,Floria1998}, and DNA dynamics \cites{Yomosa1983,Yakushevich1998,DeLeo2011}. (See \cite{braun2004}*{Chapter 2} for more physical applications of this model). Another example of  substantial interest is the discrete $\phi^4$ model
...
\revised{ (We note that in \cref{fig:SGkinks}, the kink for the continuum sine-Gordon equation connects 0 to $2 \pi$, whereas the kink for the discrete sine-Gordon equation connects $-\pi$ to $\pi$. This is explained in \cref{sec:bg} below).}
...
\begin{enumerate}[(i)]
	\item \revised{$f(u)$ is an odd function with $f'(0) < 0$. This implies that $f(0) = 0$, so 0 is an equilibrium of \cref{eq:KG}.}
	...
	left and middle-right panels have different format. RP: This was chosen for symmetry to make the analysis easier, and so that the analysis handles both s-G and $\phi^4$.}
	...
	the other two(??). RP: Hopefully this is clearer now. I explicitly state in (i) that 0 is an equlibrium.}
\end{enumerate}
Important versions include the discrete sine-Gordon equation, where $f(u) = -\sin(u)$, and the $\phi^4$ model, where $f(u) = -u(1-u^2)$.
\revised{We note that the discrete sine-Gordon equation is typically written with the nonlinearity $f(u) = \sin u$, in which case the pair of equilibria in (ii) are at 0 and $2 \pi$. This is identical except for a translation of $u_n$ by $\pi$. Choosing $f$ to have odd symmetry greatly simplifies the analysis, and allows the result to apply to both the discrete sine-Gordon equation and the $\phi^4$ model}.
...
\revised{Since $K(n)$ is a minimizer of the energy functional \cref{eq:energyfunctional}, the spectrum of $K(n)$ lies on the imaginary axis~\cite{KevrekidisWeinstein2000}*{Section 2.1.6}.}
...
class of solutions)? RP: I changed this to refer to minimizing the energy functional \cref{eq:energyfunctional}, and supplied a reference that shows that this implies the spectrum is imaginary.}
...
\end{equation}
In particular, there is a gap in the continuous spectrum $i\left(-\sqrt{f'(S^+)},\sqrt{f'(S^+)}\right)$ which contains the origin. The continuous spectrum will be the same for the linearization about any equilibrium solution. 

The continuum Klein-Gordon equation $u_{tt} = u_{xx} - f(u)$ has an eigenvalue at 0 due to translation invariance which is referred to as the Goldstone mode. Since the discrete Klein-Gordon equation does not possess any continuous symmetries, there will be no eigenvalues at the origin. Instead, there will be a symmetric pair of eigenvalues, which is either real or purely imaginary. For the inter-site kink we are considering, this pair will be imaginary since the entire spectrum is imaginary (it will be real for the on-site kink). This pair of eigenvalues is often termed Goldstone modes by extension of the Goldstone mode of the continuum equation, since these eigenvalues approach the origin as the discrete equation approaches the continuum equation, i.e. $d \rightarrow \infty$. For specific nonlinearities $f(u)$ and certain values of the coupling parameter $d$, there may be additional eigenvalues, known as internal modes, which lie between the bands of the continuous spectrum. For the kinks which are minimizers of \cref{eq:energyfunctional}, these will also be purely imaginary. (See \cite{cretegny,KevrekidisWeinstein2000}
%, in particular Figures 2 and 4, for a graphical illustration 
for a discussion of the spectrum of the kink solution, including the internal modes, for the discrete sine-Gordon equation and the $\phi^4$ model). For now, we will make the additional assumption that all point spectrum for the primary kink $K(n)$ lies in the continuous spectrum gap, i.e. all eigenvalues satisfy the inequality $|\lambda| < \sqrt{f'(S^+)}$. This assumption is proved in the next section.
...
derivative of f). You 'll need to change one or the other. RP: I renamed the potential $P$, since I don't use it very many times. In addition, I replaced $V''(u)$ with $f'(u)$ and $V'(u)$ with $f(u)$ to write everything in terms of the $f$ instead of the potential.}
\[
B = \frac{1}{d}
...
discerned from data within this fig. Do you see what I mean? RP: I had truncated the domain for the plots. I have increased the plot domain for the edge mode, so we can see it decays to 0, but much more slowly than the Goldstone mode.}
...
We can compare the eigenvalues obtained from numerical computation with those predicted by the \cref{corr:m23}. The left panel of \cref{fig:kakeigerror} plots the log of the relative error 
\[
\frac{ | \lambda_{\text{true}} - \lambda_{\text{predicted}} | }{ | \lambda_{\text{true}} | }
\]
of the two Goldstone eigenvalues vs. the separation distance $N$; the value for $\lambda$ computed with MATLAB is used as the true value of $\lambda$.
{\bf PGK: Ross, can you define more precisely the relative
error that you are computing? I think this would be beneficial
to the reader. RP: added above.}
...
to the theoretical prediction? RP: I could say that this is better than the theoretical prediction, which should be $\mathcal{O}(r_0^{-N})$, but I am not sure it is worth having to try and explain this.}
...
a better idea to do so. RP: good idea.}
...
\begin{figure}[H]
	...
	perhaps the *energy* difference, which is more relevant here? RP: By energy do you mean the Hamiltonian \cref{eq:H}? The idea here was to show that the on-site kink-antikink, when perturbed, settles into oscillatory behavior about the intersite kink-antikink.}
